...
interface AryaSamajRepository {
  ...
  suspend fun updateAryaSamaj(id: String, formData: AryaSamajFormData): Flow<Result<Boolean>>
}

class AryaSamajRepositoryImpl(private val apolloClient: ApolloClient) : AryaSamajRepository {

  override suspend fun getAryaSamajs(): Flow<Result<List<AryaSamajListItem>>> = flow {
    emit(Result.Loading)
    val result = safeCall {
      ...
      response.data?.aryaSamajCollection?.edges?.map { edge ->
        ...
        val formattedAddress = if (address != null) {
          buildString {
  ...
  override suspend fun getAryaSamajDetail(id: String): Flow<Result<AryaSamajDetail>> = flow {
    emit(Result.Loading)
    val result = safeCall {
      ...
      val addressData = if (address != null) {
        AddressData(
          location = if (address.latitude != null && address.longitude != null) {
      ...
      val members = aryaSamajNode.samajMemberCollection?.edges?.map { edge ->
  ...
  override suspend fun createAryaSamaj(formData: AryaSamajFormData): Flow<Result<String>> = flow {
    emit(Result.Loading)
    val result = safeCall {
      // Step 1: Create address
      val addressResponse = apolloClient.mutation(
      ...
      val addressId = addressResponse.data?.insertIntoAddressCollection?.records?.firstOrNull()?.id
      ...
      val aryaSamajResponse = apolloClient.mutation(
      ...
      val aryaSamajId = aryaSamajResponse.data?.insertIntoAryaSamajCollection?.records?.firstOrNull()?.id
        ?: throw Exception("Failed to get Arya Samaj ID")

      // Step 3: Add members to Arya Samaj if any
      if (formData.membersState.hasMembers) {
        val samajMemberInputs = formData.membersState.members.map { (member, postPriority) ->
        ...
        val membersResponse = apolloClient.mutation(
  ...
  override suspend fun deleteAryaSamaj(id: String): Flow<Result<Boolean>> = flow {
    emit(Result.Loading)
    val result = safeCall {
  ...
  override suspend fun updateAryaSamaj(id: String, formData: AryaSamajFormData): Flow<Result<Boolean>> = flow {
    emit(Result.Loading)
    val result = safeCall {
      ...
      val currentAryaSamaj = currentDetailResponse.data?.aryaSamajCollection?.edges?.firstOrNull()?.node
      ...
      if (currentAddress != null) {
        val addressChanged = currentAddress.basicAddress != newAddressData.address ||
        ...
        if (addressChanged) {
          val updateAddressResponse = apolloClient.mutation(
        ...
        val createAddressResponse = apolloClient.mutation(
          CreateAddressMutation(
      ...
      val aryaSamajChanged = currentAryaSamajFields.name != formData.name ||
      ...
      if (aryaSamajChanged) {
        val updateAryaSamajResponse = apolloClient.mutation(
      ...
      val newMemberMap: Map<String, Pair<String, Int>> = newMembers.map { (member, postPriority) ->
      ...
      currentMembers.forEach { currentMember ->
        ...
        if (currentMemberId != null && !newMemberMap.containsKey(currentMemberId)) {
          val removeMemberResponse = apolloClient.mutation(
      ...
      newMembers.forEach { (member, postPriority) ->
        ...
        if (currentMember == null) {
          // Add new member
          val addMemberResponse = apolloClient.mutation(
          ...
          if (currentMember.post != post) {
            val updatePostResponse = apolloClient.mutation(
          ...
          if (currentMember.priority != priority) {
            val updatePriorityResponse = apolloClient.mutation(
